...
def inference_with_perturbation(trainer, list_patient_dirs, save_path, do_TTA=True):
    ...
    with torch.no_grad():
        ...
        for patient_dir in tqdm(list_patient_dirs):
            ...
            point_set = find_boundary_points(dict_images["BrainStem_resized"])
            # At this stage, do perturbation on the OAR boundary.
            for point in point_set[-50:]:

                dict_images["BrainStem_resized"] = dilate_at(
                    dict_images["BrainStem_resized"], point
                )
                ...
                templete_nii = sitk.ReadImage(patient_dir + "/BrainStem_resized.nii.gz")
                perturbed_nii = sitk.GetImageFromArray(
                    dict_images["BrainStem_resized"][0, :, :, :]
                )
                perturbed_nii = copy_sitk_imageinfo(templete_nii, perturbed_nii)
                sitk.WriteImage(
                    perturbed_nii,
                    save_path
                    + "/"
                    + patient_id
                    + "/BrainStem_perturbed_"
                    + str(point[0])
                    + "_"
                    + str(point[1])
                    + "_"
...
if __name__ == "__main__":
    ...
    output_dir = os.path.join(root_dir, "deep-planner/output_perturb")
